...
try {
    ...
    // ID do usuário admin logado
    $userId = $_SESSION['user_id'];

    // Consultas para contar chamados por status (apenas para o admin logado)
    $sql = "SELECT status, COUNT(*) AS count 
            FROM chamados 
            WHERE finalizado_por = :user_id 
            GROUP BY status";
    ...
    // Consulta para os tipos de solicitação (apenas para o admin logado)
    $sql = "SELECT tipo_solicitacao, COUNT(*) AS count 
            FROM chamados 
            WHERE finalizado_por = :user_id 
    ...
    // Consultas para os chamados por status (apenas para o admin logado)
    $statusChamados = [];
    $statusArray = ['em_atendimento', 'atrasado', 'concluido']; 

    foreach ($statusArray as $status) {
        $sql = "SELECT chamados.*, users.nomeCompleto AS nome_admin 
                FROM chamados 
                LEFT JOIN users ON chamados.finalizado_por = users.id 
                WHERE chamados.status = :status 
                AND chamados.finalizado_por = :user_id"; // Filtrar pelo ID do admin
        $stmt = $pdo->prepare($sql);
    ...
    if (!empty($searchTerm)) {
        $sql .= " AND (chamados.id LIKE :search OR chamados.titulo LIKE :search OR chamados.descricao LIKE :search)";
    }

    // Adicionando filtro por status
    if (!empty($statusFiltro)) {
        $sql .= " AND chamados.status = :status";
    }

    // Adicionando filtro por prioridade
    if (!empty($prioridadeFiltro)) {
        $sql .= " AND chamados.prioridade = :prioridade";
    }
...
.status-icon.in-progress { background-color: #4ECF70; }
.status-icon.closed { background-color: #79C8FA; }
.status-icon.total { background-color: #3498DB; }
.status-icon.atraso { background-color: #E71D20; }
.status-icon {
...
<div class="content">
    <div class="card">
        <div class="container mt-4">
            ...
            <h3><i class="fas fa-ticket-alt"></i> Meus Chamados</h3>
            <form method="GET" action="meuschamados.php">
                <div class="row align-items-end mb-3">
                    ...
                    <div class="col-md-3 mb-2">
                        ...
                        <a href="meuschamados.php" class="btn btn-secondary">
            ...
            <div class="filter-section" onclick="toggleSection('emAtendimento', this)">
                <h5><i class="fas fa-tools icon" style= "color: #ec8d09;"></i>Pendentes Para Mim </h5>
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        <?php foreach ($statusChamados['em_atendimento'] as $chamado): ?>
                            <?php
                            // Cálculo do SLA para cada chamado (reutilizando o código que você já tinha)
                            $slaStatus = 'N/A';
                            $slaClass = '';
            ...
            <div class="filter-section" onclick="toggleSection('concluidos', this)">
                <h5><i class="fas fa-check-circle icon" style= "color: #0da074;"></i>Meus Encerramentos</h5>
                <i class="fas fa-chevron-down arrow"></i>
            </div>
            ...
            <script>
                ...
                const statusPieChart = new Chart(document.getElementById('statusPieChart'), {
                    type: 'pie',
                    data: {
                        labels: [ 'Em Atendimento',  'Em Atraso', 'Concluído'],
                        datasets: [{
                            ...
                            backgroundColor: ['#4ECF70', '#E71D20', '#79C8FA'],
                        }]
                    },
                });
                ...
                window.onload = function() {
                    const sections = ['concluidos', 'atrasado', 'emAtendimento']; // IDs das seções
                ...
                function filterTable() {
                    ...
                    const sections = ["concluidos", "atrasado", "emAtendimento"]; // Adicione IDs das seções
                    sections.forEach(sectionId => {
