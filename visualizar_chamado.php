...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $anexos = [];
    $is_admin = ($_SESSION['role'] === 'admin'); // Definir is_admin corretamente
    ...
    if (isset($_FILES['anexo']) && is_array($_FILES['anexo']['name'])) {
        ...
        foreach ($_FILES['anexo']['tmp_name'] as $key => $tmp_name) {
            if ($_FILES['anexo']['error'][$key] === UPLOAD_ERR_OK) {
                $fileInfo = pathinfo($_FILES['anexo']['name'][$key]);
                $fileExtension = strtolower($fileInfo['extension']);
                
                if (in_array($fileExtension, $allowedExtensions) && $_FILES['anexo']['size'][$key] <= $maxFileSize) {
                    $fileName = uniqid() . '_' . basename($_FILES['anexo']['name'][$key]);
    ...
    // Inserir mensagem com anexos
    if (!empty($mensagem) || !empty($anexos)) {
        try {
            $pdo->beginTransaction();
            
            $anexosString = !empty($anexos) ? implode('|', $anexos) : null;
            
            // Inserir mensagem no banco
            $stmt_send = $pdo->prepare('INSERT INTO mensagens (ticket_id, sender_id, mensagem, anexo) VALUES (?, ?, ?, ?)');
            $stmt_send->execute([$ticket_id, $sender_id, $mensagem, $anexosString]);
            ...
            if ($is_admin) {
                ...
                // Se o usuário normal enviar a mensagem, notificar todos os admins
...
<head>
    ...
    <style>
        ...
        .message img {
            width: 50px; /* Largura do ícone do usuário */
            height: 50px; /* Altura do ícone do usuário */
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-6">
                    ...
                    <?php if (!empty($planta)): ?>
                          <p><strong>Planta:</strong> <?php echo htmlspecialchars($planta); ?></p>
                    <?php endif; ?>
    ...
    <?php foreach ($mensagens as $mensagem): ?>
        <div class="message">
            ...
            <div class="message-content">
                ...
                <?php if (!empty($mensagem['mensagem'])): ?>
                    <p><?php echo nl2br(htmlspecialchars($mensagem['mensagem'])); ?></p>
                <?php endif; ?>
                
                <?php 
                if (!empty($mensagem['anexo'])) {
                    $anexos = explode('|', $mensagem['anexo']);
                ...
                ?>
                    <div class="message-attachment">
                        <a href="<?php echo htmlspecialchars($anexo); ?>" target="_blank">
                            <i class="fas <?php echo $icon; ?>"></i>
                            Anexo: <?php echo basename($anexo); ?>
                        </a>
                    </div>
                <?php 
                    endforeach;
...
<?php if ($ticket['status'] !== 'concluido'): ?>
    <form method="post" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="file" class="form-control-file" id="anexo" name="anexo[]" multiple accept=".jpg,.jpeg,.png,.pdf,.doc,.docx">
            <small class="form-text text-muted">Formatos permitidos: JPG, PNG, PDF, DOC, DOCX. Você pode selecionar vários arquivos.</small>
        </div>
...
<?php if ($ticket['status'] === 'concluido'): ?>
    <div class="card">
        ...
        <div class="avaliacao">
         <?php
          // Recupera a avaliação do chamado (agora sem filtrar pelo usuário atual)
          $stmt_avaliacao = $pdo->prepare('SELECT a.nota, a.comentario, u.username FROM avaliacoes a 
                                    JOIN users u ON a.user_id = u.id 
                                    WHERE a.chamado_id = ?');
            $stmt_avaliacao->execute([$ticket['id']]);
            $avaliacao = $stmt_avaliacao->fetch(PDO::FETCH_ASSOC);

            if ($avaliacao): 
            $nota = (int) $avaliacao['nota'];
            $comentario = $avaliacao['comentario'];
            $avaliador = $avaliacao['username'];
          ?>
          <p><strong>Avaliado por:</strong> <?php echo htmlspecialchars($avaliador); ?></p>
            <div class="estrelas">
                <?php for ($i = 1; $i <= 5; $i++): ?>
                    <span class="estrela <?= $i <= $nota ? 'preenchida' : '' ?>">★</span>
                <?php endfor; ?>
            </div>
            <?php if (!empty($comentario)): ?>
                <div class="comentario-avaliacao mt-2">
                    <p><strong>Comentário:</strong> <?php echo htmlspecialchars($comentario); ?></p>
                    </div>
                <?php endif; ?>
            <?php else: ?>
                <p>Chamado ainda não foi avaliado.</p>
            <?php endif; ?>
        </div>
    </div>
<?php else: ?>
